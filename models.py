...
class CarParkConfig:
    ...
    def get_effective_daytime_spaces(self) -> int:
        """Return number of spaces available for daytime use,
        accounting for overnight cars occupying spaces."""
        overnight = min(self.overnight.num_overnight_cars, self.total_spaces)
        return max(0, self.total_spaces - overnight)
